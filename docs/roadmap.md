...
- [x] Frontend — primer avance (v0.1.0, 2026-07-15): login + shell (sidebar/topbar, RBAC flexible), búsqueda de cliente por DNI + historial de pedidos, alta y seguimiento de gestión, alta de venta + medios de pago múltiples + aprobación (con aviso de que la mutación en VTEX sigue siendo manual), página Seguimiento con KPIs. Probado end-to-end en modo demo (`CFG.isMock()`, ver `src/data/*.json`) — sin errores de consola. Pendiente: subida real de comprobantes a Drive, detalle de gestión en drawer (hoy es un toast), y probar contra el backend real.
- [x] Frontend: pantalla de Configuración (usuarios/roles/permisos) — 2026-07-15, adaptada de `commerce-hub` (sin la pestaña "Integraciones"), ver `src/js/auth.js` (`renderUserManagementSection()`). Solo se ejerce con sesión de Administrador real conectada al backend (no en modo demo).
